...
import { Search, Button, Header, Icon, Image, Menu, Segment, Sidebar } from 'semantic-ui-react';
...
class App extends Component {
  ...
  resetComponent = () => this.setState({ isLoading: false, results: [], contacts: source, value: '' })
  ...
  handleSearchChange = (e, { value }) => {
    ...
    setTimeout(() => {
      ...
      const isMatch = result => re.test(result.general.firstName)
      let res = _.filter(source, isMatch)
      const endData = []
      const data = _.map(res, value => {
        endData.push({ title: value.general.firstName })
      })
  ...
  render() {
    const { isLoading, value, results, contacts } = this.state;
